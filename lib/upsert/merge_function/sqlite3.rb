  ...
  class MergeFunction
    ...
    module Sqlite3
      ...
      def initialize(*)
        ...
        @quoted_update_names = setter_keys.select { |k| k !~ CREATED_COL_REGEX && !options["ignore_on_update"].include?(k) }.map { |k| connection.quote_ident k }
      ...
      def execute(row)
        ...
        bind_update_values = row.setter.select{ |k,v| k !~ CREATED_COL_REGEX && !options["ignore_on_update"].include?(k) }.map { |k,v| connection.bind_value v }
